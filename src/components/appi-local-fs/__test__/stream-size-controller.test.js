...
test('StreamSizeController should pass through stream if its size is ok', async () => {
    ...
    return new Promise((resolve, reject) => {
        ...
        restrictedSourceStream.on('error', (err) => reject(err))
        targetStream.on('finish', () => process.nextTick(() => resolve()))

    })

})

test('StreamSizeController should emit error if stream size is too large', async () => {
